...
## Abstract
We developed an interest point detector to detect interest point location for event camera images. The idea was adapted from the literature *"SuperPoint: Self-Supervised Interest Point Detection and Description."* Daniel DeTone, Tomasz Malisiewicz, Andrew Rabinovich. ArXiv 2018. 

Our algorithm mainly consists of two parts:
We first train our detector network with a synthetic dataset. We then iteratively use the trained detector network to label interest points of a real-world dataset and train the network with the labeled dataset.
